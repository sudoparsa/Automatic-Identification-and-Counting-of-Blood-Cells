...
def blood_cell_count(file_name):
    ...
    for prediction in output:
        ...
        if label == 'RBC' and confidence < .125:
            continue
        if label == 'WBC' and confidence < .0625:
            continue
        if label == 'Platelets' and confidence < .0625:
            continue
        ...
        image = cv2.putText(image, label + ' ' + str(confidence), (center_x - 15, center_y + 5), font, .5, color, 1)
